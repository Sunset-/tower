...
// 获取文件夹目录
const getModuleDirs = function(mypath = ".") {
	mypath = `./${mypath}/src`;
	const items = fs.readdirSync(mypath);
	let result = [];
	// 遍历当前目录中所有的文件和文件夹
	items.map((item) => {
		let itemPath = path.join(mypath, item);
        if(item == "common") {
            // 编译整个文件夹
            ...
            if (fs.statSync(itemPath).isDirectory()) {
                // 读取子文件夹
                const modules = fs.readdirSync(itemPath);
                modules.map((module) => {
                    if (fs.statSync(path.join(itemPath, module)).isDirectory()) {
                        result.push({
                            name: `${item}/${module}`,
                            value: `${item}/${module}`,
                        });
