<template>
    <xui-fasttree v-if="treeOptions" ref="tree" class="dag-tree" :options="treeOptions" @init="onInited"
        @selected-node="onSelected" @checked="onChecked" @cancel-checked="onCancelChecked"></xui-fasttree>
</template>
...
export default {
    ...
    methods: {
        ...
        initTreeOptions() {
            ...
            if (Object.keys(safeOptions).length != 0) {
                ...
                var treeOptions = {
                    ...
                    check:
                        typeof safeOptions.check == "object"
                            ? safeOptions.check
                            : {
                                  ...
                                  cascade:
                                      (safeOptions.check &&
                                          safeOptions.check.cascade) ||
                                      "C",
                              },
                    levelSpace: 20,
                    nodeHeight: 30,
                    doubleClickInterval: 0,
                    isAutoExpandRoot: safeOptions.isAutoExpandRoot===false?false:true,
                    isStrongCheckedMode: !!safeOptions.isStrongCheckedMode,
                    // isExpandOnDbClickNode: true,
                    template: safeOptions.template
                        ? safeOptions.template
                        : (node) => {
                              var icon;
                              if (node.type == "group") {
                    ...
                    <span style="${
                        node.isLive ? "color:#4CAF50" : ""
                    }" class="dag-tree-node-label">${
                                  safeOptions.nodeTemplate
                                      ? safeOptions.nodeTemplate(node)
                                      : node.name
                              }</span>
                    </div>`;
                          },
                    // search: (filter) => {
                    //     if ($config.get("singleNode")) {
                    //         return this.loadNodes(
                    //             Store.singleSearch(filter, orgType)
                    //         );
                    //     } else {
                    //         return this.loadNodes(
                    //             Store.search(filter, orgType)
                    //         );
                    //     }
                    // },
                    isParent(node) {
                ...
                if (safeOptions.isParent) {
                    treeOptions.isParent = safeOptions.isParent;
                }
                ...
                if (safeOptions.datasource) {
                    treeOptions.datasource = safeOptions.datasource;
                } else {
                    switch (treeType) {
                        case 1:
                            treeOptions.nodes = () => {
        ...
        selectNode(id, bool = true) {
            this.$refs.tree && this.$refs.tree.selectNode(id, bool);
        },
        ...
        onChecked(nodes) {
            ...
            var resources =
                nodes &&
                nodes.filter((item) => item.nodeType && item.nodeType != 1);
...
.dag-tree {
    ...
    &.xui-fasttree {
        width: 100%;
        /deep/ .xui-fasttree-nodes {
            width: 100%;
        }
        /deep/ .xui-fasttree-node-empty {
            display: none;
            height: @node-height;
