...
export default {
    ...
    watch: {
        params(v) {
            this.$refs.form.reset(
                this.params[`DIChannelPar_${this.channelIndex}`] &&
                    this.params[`DIChannelPar_${this.channelIndex}`].params
            );
    ...
    computed: {
        channel() {
            return `DIChannelPar_${this.channelIndex}`;
        },
    },
    data() {
        return {
    ...
    methods: {
        save(model) {
            ...
            if (model.ChannelIndex == "0") {
                model.RangeDown = model.RangeDown || 0;
                model.RangeUp = model.RangeUp || 0;
            } else {
                model.RangeDown = model.RangeDown || 0;
                model.RangeUp = model.RangeUp || 0;
                model.PowerSwitch = model.PowerSwitch || 0;
                model.PowerDelayTim = model.PowerDelayTim || 0;
                model.PowerSelect = model.PowerSelect || 0;
            }

            var type = `DIChannelPar_${this.channelIndex}`;
            this.$emit("submit", {
        ...
        workingTitle(model, options) {
            var v =
                this.workingParams[`DIChannelPar_${this.channelIndex}`] &&
                this.workingParams[`DIChannelPar_${this.channelIndex}`].params[
                    options.name
                ];
            if (options.data) {
