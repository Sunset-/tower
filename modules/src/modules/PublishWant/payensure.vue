    ...
    <div class="timeline-panel wechat-order-payensure" title="申请付款">
        <div class="info-wrap">
            ...
            <div class="customer-img" v-for="img in images" :key="img.id">
                ...
                <img src="/images/icon/pdf_icon.jpg" @click="viewImage(img,images)" :title="img.name">
                <div class="img-label" :title="img.name">{{img.name}}</div>
            </div>
        </div>
        <div class="info-wrap info-current"
            style="padding-left:5px;margin-top:15px;border-top:1px solid #ababab;padding-top:15px;">
            <xui-button size="big" style="width:200px;" color="danger" @click="submit">结束流程</xui-button>
        </div>
    </div>
</template>
<script>
