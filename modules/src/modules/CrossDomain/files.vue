<template>
    <div class="timeline-panel wechat-order-files" :title="title+' - 法律文书'+`(${images.length})`">
        <!-- 核算 -->
        <div class="info-group order-claim">
            <div class="timepanel-operate">
                <xui-button class="operate-btn" :color="color" @click="submit()">{{text}}</xui-button>
            </div>
            <div class="timepanel-toolbar">
                <xui-file ref="uploader" :options="uploadOptions" :ctx="{category : title}" @success="uploadSuccess">
                </xui-file>
