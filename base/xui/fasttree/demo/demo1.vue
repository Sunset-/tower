<template>
	<div>
		...
		<np-fasttree ref="tree" class="fasttree-demo" :options="options" @init="inited" @click-node="selected" @checked="checkedNodes" style="height:200px;"></np-fasttree>
...
window.changeSelect = function changeSelect(){
	debugger;
}
export default {
	methods: {
		...
		inited() {
			...
			this.$refs.tree.checkNodes(['1_1','1_2'])
		},
		...
		return {
			...
			options: {
				...
				template(node,$tree) {
					return `<div class="demo-node" style="height:30px;"><i class="xui-icon xui-icon-tasklist_fill"></i><span>${
						node.name
					}</span><select class="xui-fasttree-operate" onChange="changeSelect()">
			...
			treeNodes: (() => {
				return Promise.resolve().then(() => {
					var map = SukiiXUI.Dictionary.getAllEnums(),
						nodes = [];
