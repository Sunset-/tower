...
FastTree.prototype = {
	 ...
	 */
	initScroll() {
		this.unlistenResize = DomHelper.listenResize(this.$wrap, () => {
			this._updateBar();
		});
		this._updateBar();
			...
			if (!lastClickTarget) {
				...
				clickTimer = setTimeout(
					() => {
						...
						this._emit("click-node", node, this.getAncestors(nodeId));
					},
					Sunset.isNumber(this.options.doubleClickInterval) ? this.options.doubleClickInterval : 350
				);
				...
				this._emit("dbclick-node", node, this.getAncestors(nodeId));
			}
		});
		//drag
		var dragging = false;
	...
	getAncestors(id) {
		...
		var parents = [];
		var parent = id_node[id];
		while (parent) {
			var parent = id_node[parent[pIdKey]];
			if (parent) {
	...
	_template(node, level, ancestorIds) {
		...
		var html = [],
			isParent = !node.__FASTREENODE && Sunset.isFunction(this.options.isParent) && this.options.isParent(node),
			id = node[idKey];
		html.push(
	...
	_calculateLevel(index, nodes, lastLevel) {
		...
		if (nodes[index]._fromSearch && !this.options.searchBuildTree) {
			return 0;
		}
	...
	_calculateCheckedStatus(node, ancestorIds) {
		...
		if (id === "__CHECK_ALL__") {
			let length = this.nodes.filter(n => n[idKey] != "__CHECK_ALL__" && !this.checkedIds[n[idKey]]).length
			return length ?
				(length == this.nodes.length - 1 ? "" : "half-check") :
				"checked";
		}
	...
	_isChildrenAllChecked(id) {
		...
		if (childrenIds && childrenIds.length > 0) {
			for (var i = 0, l = childrenIds.length; i < l; i++) {
				...
				if (this.parentId_childrenIds[childId]) {
					if (!this._isChildrenAllChecked(childId)) {
						return false;
					}
			...
			.then(res => {
				...
				if (this.options.isAutoExpandRoot) {
					if (roots && roots.length) {
						return Promise.resolve(this.expandNode(roots[0])).then(() => {
	...
	generateVirtualTree(nodes, fillEmpty) {
		...
		for (var i = 0, item;
			...
			if (fillEmpty) {
				...
				if (!pid_cidMap[pId]) {
					var cmap = {};
					for (var j = 0, l = cids.length; j < l; j++) {
						cmap[cids[j]] = true;
					}
					pid_cidMap[pId] = cmap;
				}
	...
	selectNode(id, flag) {
		...
		var node = this.getNodeById(id);
		this._emit("selected-node", node, this.getAncestors(id));
	},
	clearSelected() {
		this.selectIds = {};
		this.render();
	...
	checkNode(id, isToChecked, slient) {
		if (id === "__CHECK_ALL__") {
			isToChecked = false;
			var ids = this.nodes
				.filter(item => item[this.idKey] != "__CHECK_ALL__" && this._enableCheck(item))
				.map(item => {
			...
			if (isToChecked) {
				Object.keys(this.checkedIds).forEach((i) => {
					ids.push(i)
				})
				this.checkNodes(ids);
			} else {
				let checkids = Object.keys(this.checkedIds).filter((item) => { return !ids.includes(item) })
				this.clearChecked();
				this.checkNodes(checkids);
			}
			return;
		}
		Promise.resolve()
			.then(() => {
				var isChecked = this.checkedIds[id],
					reRender = false;
				if(!isChecked&&!this._isLeaf(id)&&this._isChildrenAllChecked(id)&&!this.options.isStrongCheckedMode){
					isChecked = true;
				}
				if (this.isCheckMultiple) {
					//多选
			...
			.then(res => {
				...
				if(!this.isCheckCascdeChildren&&!this.isCheckCascdeParent){
					return;
				}
				var self = this;
				return Promise.resolve().then(()=>{
					if (this.isCheckCascdeChildren) {
						...
						function cascadeCheckChild(childrenIds) {
							if (childrenIds) {
								childrenIds.forEach(cid => {
									if (isToChecked) {
										if (
											(!self.searchFilter || self.searchFilter(self.id_node[cid])) &&
											self._enableCheck(self.id_node[cid])
										) {
						...
						function cascadeCheckParent(cid) {
							if (!isToChecked) {
								...
								while(true){
									var cnode = self.id_node[cid];
									var pid = cnode[self.pIdKey];
									if (self.checkedIds[pid]||pid==self.rootId) {
										return;
									}
									self.checkedIds[pid] = true;
									cid = pid;
								}
							}
						}
						...
						return Promise.resolve().then(() => {
							var node = this.getNodeById(id);
							if (node) {
								var loadAllParents = false;
								var pId = node[this.pIdKey];
								while (true) {
									if (pId == this.rootId) {
										loadAllParents = true;
										break;
									...
									pId = pNode[this.pIdKey];
								}
								if (!loadAllParents) {
									if (Sunset.isFunction(this.options.loadAncestors)) {
										return this.options.loadAncestors(id).then(res => {
											if (res && res.length) {
												res.map(n => {
						...
						return this.options.loadAncestors(unloadCheckedIds.join(",")).then(res => {
							if (res && res.length) {
								if (res.length <= 10) {
									...
									return this._loadPosterity(this.rootId);
	...
	clearChecked(slient) {
		this.checkedIds = {};
		this.render();
		slient || this._emitCheckedNodes();
	},
	_emitCheckedNodes() {
		var nowCheckIds = {};
		var checkedNodes = [];
		var checkedIds = Object.keys(this.checkedIds);
		for (var i = 0, l = checkedIds.length; i < l; i++) {
			var id = checkedIds[i];
			var node = this.getNodeById(id);
			if (node && !node.__FASTREENODE) {
				if (node.$parent === void 0) {
					node.$parent = this.getNodeById(node[this.pIdKey]) || null;
				}
				checkedNodes.push(node);
				nowCheckIds[id] = true;
			}
		}
		// console.log("fast-tree-check : " , checkedNodes);
		//计算取消勾选的ids
		var lastCheckedIds = Object.keys(this.lastCheckedIds);
		var removeCheckedNodes = [];
		for (var i = 0, l = lastCheckedIds.length; i < l; i++) {
			var id = lastCheckedIds[i];
			var node = this.getNodeById(id);
			if (node && !node.__FASTREENODE && !nowCheckIds[id]) {
				removeCheckedNodes.push(node);
			}
		}
		this.lastCheckedIds = nowCheckIds;
		this._emit(
			"checked",
			checkedNodes,
		...
		this._emit("cancel-checked", removeCheckedNodes);
		// console.log("fast-tree-cancel-check : " ,removeCheckedNodes);
	},
	/**
	 * 级联获取子孙节点
	 */
	...
	search(keyword, force) {
		// 多个筛选条件传值对象
		if (Sunset.isObject(keyword)) {
			keyword = Object.values(keyword).filter((item) => {
				return item
			}).length == 0 ? "" : keyword
		}
		keyword = keyword || "";
			...
			if (Sunset.isFunction(this.options.search)) {
				if (force || keyword) {
					...
					this.options.search(keyword).then(res => {
						...
						if (this.isCheckMultiple && res.length && !this.options.searchBuildTree) {
							var checkAll = {};
